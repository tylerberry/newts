/*
 * nfpipe.c - write a note with text from stdin
 *
...
main (int argc, char **argv)
{
  ...
  while ((opt = getopt_long (argc, argv, "ad::ht:v",
                             long_options, &option_index)) != -1)
    {
      switch (opt)
        ...
        case 0:
          {
            printf_version_string (N_("nfpipe"));
        ...
        case 'h':
          printf (_("Usage: %s [OPTION]... [FILE] NOTESFILE\n"
                    "Create a note in NOTESFILE with text from FILE (or stdin if unspecified).\n\n"),
                  program_name);
  ...
  if (!note.title)
    note.title = newts_strdup ("** From nfpipe **");
