...
int
uiuc_open_nf (const newts_nfref *ref, struct notesfile *nf)
{
  struct io_f io;
  struct uiuc_opts *opts;
  int error;
  opts = newts_malloc (sizeof (struct uiuc_opts));
  ...
  opts->notesfile_number = io.descr.d_nfnum;
  opts->current_note_id = io.descr.d_id.uniqid;

  nf->perms = io.access;
  nf->opts = (struct opts *) opts;

  time (&nf->time_entered);
