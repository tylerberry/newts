/*
 * gl_getline.c - getline input editing processor
          ...
          switch (c)
            {
              ...
              /* Added by Tyler Berry, to support the vi
               * delete character.
               */
      ...
      else
    {
      ...
      switch (c)
          ...
          if (gl_completion_proc)
        {
          ...
          loc =
            gl_do_tab_completion (gl_buf, &tmp, sizeof (gl_buf),
                      (lastch == '\t'));
          gl_buf[sizeof (gl_buf) - 1] = '\0';
          if (loc >= 0 || tmp != gl_pos)
          ...
          else if (gl_tab_hook)
        {
          ...
          gl_buf[sizeof (gl_buf) - 1] = '\0';
          loc =
            gl_tab_hook (gl_buf, (int) gl_strlen (gl_prompt), &tmp,
                 sizeof (gl_buf));
          gl_buf[sizeof (gl_buf) - 1] = '\0';
          if (loc >= 0 || tmp != gl_pos)
 ...
 */
{
  ...
  if (change == -2)
    {               /* reset */
      ...
      /* This change was made to allow Newts to prompt at arbitrary locations
       * on the screen. If you're using this code in another application, you
       * might want to uncomment this line (and the same line in the next
       * section.
       */
...
gl_display_matches(int nused)
{
  ...
  if (nused == 0)
    ...
    {
      qsort (gl_matchlist, (size_t) nused, sizeof (char *),
         gl_display_matches_sort_proc);
...
#define GL_COMPLETE_VECTOR_BLOCK_SIZE 64
  ...
  newgl_matchlist =
    (char **) malloc ((size_t) (sizeof (char *) * (ntoalloc + 1)));
      ...
      for (;;)
    {
      ...
      if (dent == NULL)
          ...
          if (idx == 1)
        {
          ...
          /*  CHANGE: lstat => stat to search links.  */
      ...
      if ((filepfxlen == 0) || (strncmp (name, filepfx, filepfxlen) == 0))
        {
          /* match */
          len = strlen (name);
          cp = (char *) malloc (filepfxoffset + len + 1 /* spare */  +
                    1 /* NUL */ );
          *cp = '\0';
          if (filepfxoffset > 0)
...
gl_local_filename_completion_proc(const char *start, int idx)
{
  ...
  for (;;)
      ...
      if ((filepfxlen == 0) || (strnicmp (name, filepfx, filepfxlen) == 0))
    {
      /* match */
      len = strlen (name);
      cp = (char *) malloc (filepfxoffset + len + 4 /* spare */  +
                1 /* NUL */ );
      *cp = '\0';
      if (filepfxoffset > 0)
